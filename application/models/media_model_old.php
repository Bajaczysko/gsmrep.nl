...
class Media_model extends CI_Model {
			...
			foreach($media as $md) {					
				...
				if(file_exists($file_path)) {					
					...
					switch($file_ext)
                    {
                        ...
                        case 'jpeg':
                        case 'ico':                             
                            $img_url = $anchor_url = 'assets/'.$params->media_path.'/'.$user_id.'/'.$path.$md->name;
                            ...
                            if (($info[0] > $this->img_width) || ($info[1] > $this->img_width))
                            {
                                ...
                                $url = 'assets/'.$params->media_path.'/'.$user_id.'/'.$path.'thumb/'.$md->name;                                                                   
                        ...
                        default:
                            ...
                            $data[$count] = array(
                                ...
                                'file_type'   => $type[0],                                   
                                'path'        => $path.$md->name,                                   
                                'file_url'    => 'assets/'.$params->media_path.'/'.$user_id.'/'.$path.$md->name,
	...
	public function get_params()
	{
		...
		$default_params = json_decode(read_file(realpath(FCPATH . 'themes/default/media-manager/default.json')));
